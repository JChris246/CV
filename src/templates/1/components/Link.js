...
const Link = ({ iconName, text, url }) => {
    ...
    return (
        url ? (
            <a href={text} className="p-4 text-white rounded-full w-11/12 my-4 mx-auto shadow-lg glass flex pop">
                <Icon width={30} height={30}/>
                <span className="block font-thin ml-5 self-center">{text}</span>
            </a>
        ) : (
